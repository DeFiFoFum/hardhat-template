...
import { LockUpgradeable, LockUpgradeable__factory } from '../../../typechain-types'
import {
  formatNamedProxyContractName,
  SupportedNamedProxyContractNames,
} from '../../../lib/evm/proxy/namedProxy.config'

// TODO: Hardcoded using the LockUpgradeable implementation. Would be nice to infer from the named proxy contract.
async function deployCreate2VanityProxy(contractName: SupportedNamedProxyContractNames) {
  ...
  const deployProxyResult = await createXDeployer.deployProxyWithSalt('LockUpgradeableProxy', salt.salt, salt.address)
  const lockProxy = await ethers.getContractAt('ITransparentUpgradeableProxy', deployProxyResult.proxyAddress)

  logger.log(
    `LockUpgradeableProxy deployed at address: ${deployProxyResult.proxyAddress}, expected address: ${salt.address}`,
    '🚀',
  )

  const verificationCommand = `npx hardhat verify --network ${currentNetwork} ${deployProxyResult.proxyAddress} '${deployProxyResult.constructorArgs[0]}' '${deployProxyResult.constructorArgs[1]}' '${deployProxyResult.constructorArgs[2]}'`
  logger.log(`Verification command: ${verificationCommand}`, '🔎')
  ...
  logger.log(
    `Upgrading ${proxyContractName} to implementation at address: ${lockUpgradeable_Implementation.address}`,
    '🚀',
  )
  await (await lockProxy.connect(deployerSigner).upgradeTo(lockUpgradeable_Implementation.address)).wait()
  logger.log(
    `Upgraded ${proxyContractName} to implementation at address: ${lockUpgradeable_Implementation.address}`,
    '🚀',
  )

  const lockUpgradeable = (await ethers.getContractAt('LockUpgradeable', lockProxy.address)) as LockUpgradeable
  logger.log(`Changing admin of ${proxyContractName} to ${SecureAdminSafe}`, '🚀')
  await (await lockProxy.connect(deployerSigner).changeAdmin(SecureAdminSafe)).wait()
